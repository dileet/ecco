import type { PooledConnection, ConnectionPoolConfig, PoolState } from './connection-types';
...
export async function cleanup(
  ...
  debug('connections', `Cleaning up ${toRemove.length} stale connections`);
...
export async function closePool(state: PoolState): Promise<PoolState> {
  ...
  for (const conn of allConnections) {
    connections = await remove(connections, conn);
  }
