...
import { z } from 'zod';
import { verifyMessage, signMessage, type SignedMessage } from '../auth/authenticator';
      ...
      if (pubsubAbortControllers.has(pubsubKey)) {
        ...
        try {
          ...
          if (currentState.messageAuth) {
            const parsed = SignedMessageSchema.safeParse(rawData);
            if (parsed.success) {
              const { valid } = await verifyMessage(currentState.messageAuth, parsed.data);
              if (!valid) {
