...
import { LIBP2P } from './transport-constants';
...
const RemotePeerSchema = z.custom<{ toString(): string }>((val) =>
  val !== null && typeof val === 'object' && 'toString' in val && typeof (val as { toString: unknown }).toString === 'function'
);

const IncomingStreamDataSchema = z.object({
  stream: z.custom<Stream>((val) => val !== null && typeof val === 'object' && Symbol.asyncIterator in val),
  connection: z.object({
    remotePeer: RemotePeerSchema,
  }),
});

const AsyncIterableSchema = z.custom<Stream>((val) => val !== null && typeof val === 'object' && Symbol.asyncIterator in val);
...
const StreamWithConnectionSchema = z.object({
  stream: AsyncIterableSchema,
  connection: z.object({
    remotePeer: RemotePeerSchema,
  }),
});

function parseIncomingData(data: unknown): ParsedIncomingData | null {
  const structured = StreamWithConnectionSchema.safeParse(data);
  if (structured.success) {
    return {
      stream: structured.data.stream,
      remotePeerId: structured.data.connection.remotePeer.toString(),
    };
  }

  const streamOnly = AsyncIterableSchema.safeParse(data);
  if (streamOnly.success) {
    return { stream: streamOnly.data, remotePeerId: null };
  }
...
export function initialize(state: Libp2pAdapterState): Libp2pAdapterState {
  ...
  node.handle(protocol, async (incomingData) => {
    const parsed = parseIncomingData(incomingData);

    if (!parsed) {
      return;
    }

    const { stream, remotePeerId: remotePeerIdFromConnection } = parsed;
